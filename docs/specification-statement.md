...
- **Accounts and authentication — first-class V1 feature.** Robust, consumer-scale authentication ships in V1, sized for the platform's future rather than V1's modest needs: email/password registration with modern credential handling, social sign-in (OIDC-based providers), optional multi-factor authentication, secure account recovery, session and device management (including sign-out of the current session, per-session revocation, and a single-action sign-out-everywhere), and full account lifecycle (deactivation, data export, deletion). Data export produces a downloadable, machine-readable artifact (JSON) containing the user's data; any on-screen rendering is a courtesy view, not the deliverable. Account recovery covers MFA loss, not only passwords: one-time backup codes (stored as salted hashes, shown once at generation) are the self-service path, and an audited staff MFA-reset (platform-admin console) is the assisted path. Password reset must never silently bypass an MFA gate at sign-in, and MFA loss must never be a permanent lockout. On mobile, an optional biometric app-unlock (platform biometrics gating use of the locally persisted session) provides password-free day-to-day access without weakening the underlying authentication; storing or releasing the account password via biometrics is prohibited. True passwordless sign-in — including biometric re-authentication after session expiry — arrives with passkeys/WebAuthn, the anticipated strategic successor authentication method (deferred; the enrollment model already accommodates additional method types). **Date of birth is captured at registration** and stored — unused for gating in V1, but required so future age-gated consumer features activate without re-collecting from the installed base. Identity/auth is its own isolated module per the constitution.
...
- **Classification hierarchy — best-in-class requirement.** Ingredients form a taxonomy from broad class to specific product, e.g., Spirit → Rum → Rhum Agricole → _specific brand/bottling_. The spine hierarchy is single-parent and deliberately shallow: it encodes identity only, and two categories that can be simultaneously true of one product (aged, dark, Jamaican) are never tree nodes. All simultaneous characteristics are **facets** — curator-managed attributes on classes and products (age statement, color, origin/region, production style, proof class, etc.). A recipe line may reference any hierarchy level and may additionally carry **facet constraints** (e.g., class Rum + origin Jamaica + age ≥ 12), making requirements like "12-year Jamaican rum" expressible without polluting the tree. Products carry an availability status (in production / discontinued); recipe lines referencing discontinued products retain the historical reference and surface curated substitutions (e.g., the original Mai Tai's Wray & Nephew 17). **Match-quality ladder** (computable, in descending order): exact product; class-satisfied (a descendant product with facets met is a _true match_, not a substitution — Beefeater fully satisfies "London dry gin"); facet-partial (right class, a constraint unmet); curated/sibling substitution (with suitability notes); no path. Substitution exists only sideways or upward (product-for-product, or class standing in for a specified product) — never downward. Subjective quality tiers ("top shelf") are explicitly not facets; they live in recipe notes and substitution suitability language. Recipes may reference any level (a recipe calls for "London dry gin"; a user's inventory holds "Beefeater"). The hierarchy powers:
