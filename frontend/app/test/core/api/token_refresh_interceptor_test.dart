...
void main() {
  test(
    'a 401 triggers a silent refresh and retries the original request',
    () async {
      ...
      final container = ProviderContainer(
        overrides: [authDioProvider.overrideWithValue(authDio)],
      );
      ...
      apiDio.interceptors.add(
        container.read(
          Provider<TokenRefreshInterceptor>(
            (ref) => TokenRefreshInterceptor(ref, apiDio),
          ),
        ),
      );
      ...
      expect(
        protectedCallCount,
        2,
        reason: 'the original 401 call, then one retry with the fresh token',
      );
  ...
  test(
    'a failed refresh clears both tokens and propagates the original 401',
    () async {
      final apiDio = Dio(BaseOptions(baseUrl: 'http://test.invalid'));
      final authDio = Dio(BaseOptions(baseUrl: 'http://test.invalid'));
