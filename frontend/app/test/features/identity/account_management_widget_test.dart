...
void main() {
  Widget buildTestApp(_FakeAccountInterceptor interceptor, Widget screen) {
    final identityDio = Dio(BaseOptions(baseUrl: 'http://test.invalid/api/v1'))..interceptors.add(interceptor);
  ...
  group('SessionsScreen', () {
    testWidgets('lists sessions and revokes one', (tester) async {
      ...
      await tester.pumpWidget(buildTestApp(interceptor, const SessionsScreen()));
  ...
  group('AccountLifecycleScreen', () {
    testWidgets('deactivating requires confirmation, then shows the reactivate button', (tester) async {
      final interceptor = _FakeAccountInterceptor();
      await tester.pumpWidget(buildTestApp(interceptor, const AccountLifecycleScreen()));
      await tester.pumpAndSettle();

      await tester.tap(find.byKey(const Key('accountLifecycleDeactivateButton')));
      await tester.pumpAndSettle();

      expect(find.byKey(const Key('accountLifecycleConfirmDialog')), findsOneWidget);
  ...
  group('AccountDataScreen', () {
    testWidgets('exports data and shows the date of birth', (tester) async {
      final interceptor = _FakeAccountInterceptor();
      await tester.pumpWidget(buildTestApp(interceptor, const AccountDataScreen()));
      await tester.pumpAndSettle();

      await tester.tap(find.byKey(const Key('accountDataExportButton')));
      await tester.pumpAndSettle();

      expect(find.byKey(const Key('accountDataExportDateOfBirth')), findsOneWidget);
      expectNoRawLocalizationKeys(tester);
    });

    testWidgets('deleting requires confirmation, then navigates home', (tester) async {
      final interceptor = _FakeAccountInterceptor();
      await tester.pumpWidget(buildTestApp(interceptor, const AccountDataScreen()));
      await tester.pumpAndSettle();
      ...
      expect(find.byKey(const Key('accountDataDeleteConfirmDialog')), findsOneWidget);
      expect(interceptor.deleteCallCount, 0);

      await tester.tap(find.byKey(const Key('accountDataDeleteConfirmDialogButton')));
      await tester.pumpAndSettle();
...
class _FakeAccountInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    if (options.path == '/me/sessions' && options.method == 'GET') {
      ...
      return handler.resolve(Response(requestOptions: options, statusCode: 200, data: {'sessions': sessions}));
    }

    if (options.path.startsWith('/me/sessions/') && options.method == 'DELETE') {
