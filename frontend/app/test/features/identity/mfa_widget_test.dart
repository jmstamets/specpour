...
void main() {
  ...
  testWidgets(
    'signing in with MFA enabled shows the challenge screen, then a valid code signs in',
    (tester) async {
      ...
      await tester.enterText(
        find.byKey(const Key('mfaChallengeCodeField')),
        '123456',
      );
  ...
  testWidgets('an invalid MFA code shows the error and does not navigate', (
    tester,
  ) async {
  ...
  testWidgets(
    'MFA settings: enrolling shows the secret, then confirming enables it',
    (tester) async {
      final identityInterceptor = _FakeIdentityInterceptor();
  ...
  testWidgets(
    'MFA settings: disabling an enabled enrollment updates the status',
    (tester) async {
      final identityInterceptor = _FakeIdentityInterceptor(mfaEnabled: true);
        ...
        return handler.resolve(
          Response(
            ...
            data: {
              ...
              'otpAuthUri':
                  'otpauth://totp/SpecPour:test?secret=$_pendingSecret',
            },
          ),
        );
      }
