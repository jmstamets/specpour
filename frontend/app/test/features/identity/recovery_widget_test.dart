...
void main() {
  Widget buildTestApp({required _FakeIdentityInterceptor identityInterceptor, String initialLocation = '/recovery'}) {
    ...
    final router = GoRouter(
      ...
      routes: [
        ...
        GoRoute(path: '/recovery', builder: (context, state) => const RecoveryRequestScreen()),
        GoRoute(path: '/recovery/confirm', builder: (context, state) => const RecoveryConfirmScreen()),
      ],
    );

    return ProviderScope(
      overrides: [identityApiProvider.overrideWithValue(IdentityApi(identityDio, standardSerializers))],
  ...
  testWidgets('requesting recovery shows the (no-enumeration) success message', (tester) async {
    ...
    await tester.enterText(find.byKey(const Key('recoveryRequestEmailField')), 'someone@example.test');
    await tester.tap(find.byKey(const Key('recoveryRequestSubmitButton')));
    await tester.pumpAndSettle();

    expect(find.byKey(const Key('recoveryRequestSuccessMessage')), findsOneWidget);
  ...
  testWidgets('confirming recovery with a valid code navigates to sign-in', (tester) async {
    final identityInterceptor = _FakeIdentityInterceptor();

    await tester.pumpWidget(buildTestApp(identityInterceptor: identityInterceptor, initialLocation: '/recovery/confirm'));
    await tester.pumpAndSettle();

    await tester.enterText(find.byKey(const Key('recoveryConfirmEmailField')), 'someone@example.test');
    await tester.enterText(find.byKey(const Key('recoveryConfirmTokenField')), 'a-real-token');
    await tester.enterText(find.byKey(const Key('recoveryConfirmNewPasswordField')), 'a brand new passphrase');
    expectNoRawLocalizationKeys(tester);
  ...
  testWidgets('confirming recovery with an invalid token shows the error', (tester) async {
    ...
    await tester.enterText(find.byKey(const Key('recoveryConfirmEmailField')), 'someone@example.test');
    await tester.enterText(find.byKey(const Key('recoveryConfirmTokenField')), 'a-bad-token');
    await tester.enterText(find.byKey(const Key('recoveryConfirmNewPasswordField')), 'a brand new passphrase');
    ...
    expect(find.byKey(const Key('recoveryConfirmScreen')), findsOneWidget);
    expect(find.byKey(const Key('recoveryConfirmErrorMessage')), findsOneWidget);
    expectNoRawLocalizationKeys(tester);
  });
}
...
class _FakeIdentityInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    if (options.path == '/auth/recovery') {
      ...
      return handler.resolve(Response(requestOptions: options, statusCode: 202));
    }
