...
/// OWN composed host (a fresh <see cref="SpecPourWebApplicationFactory"/>, so a
/// fresh rate-limiter with its own partitions) rather than the shared
/// AcceptanceHooks.Factory — exhausting the anonymous 100-req/min budget on the
/// shared host would spuriously 429 every other anonymous scenario in the same
/// run. Reuses the already-migrated Testcontainers database via
/// AcceptanceHooks.ConnectionString.
///
